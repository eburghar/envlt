...
In case `VALUE` matches a backend expression `backend:args:path`, the value is expanded by calling the backend with
the provided arguments, and envlt recursively define one variable name based on the expanded value, joining `NAME`
and the path components `_`. Path components are keys for dictionaries and indexes (starting at 0) for arrays.
